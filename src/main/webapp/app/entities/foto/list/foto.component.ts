...
import { EntityArrayResponseType, FotoService } from '../service/foto.service';
...
export class FotoComponent implements OnInit {
  ...
  trackId = (_index: number, item: IFoto): number => this.fotoService.getFotoIdentifier(item);
  ...
  protected refineData(data: IFoto[]): IFoto[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IFoto[] | null): IFoto[] {
    return data ?? [];
  }
